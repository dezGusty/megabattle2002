...
void TEditForm::ResetariDate() {
	...
	FelErou->Text = buf;
	Merc1->Text = buf;
	UpDown1->Position = (short)Level;
	Merc2->Text = buf;
	UpDown2->Position = (short)Level;
	Merc3->Text = buf;
	UpDown3->Position = (short)Level;
	Merc4->Text = buf;
	UpDown4->Position = (short)Level;
	Merc1->Text = buf;
	UpDown1->Position = (short)Level;
	Merc1->Text = buf;
	UpDown1->Position = (short)Level;
...
void ScrieUnitate(FILE *ff, int fel, int level, int y_position) {
	...
	fprintf(ff, "%d %d %d %d %d ", movesmax, dmg_min, dmg_max, armor,
		protection);
	fprintf(ff, "%d %d %d %d %d ", chance, upkeep, imag_asoc, port_asoc,
		ranged);
...
void __fastcall TEditForm::SaveCurrentHero1Click(TObject *Sender) {
	...
	if (SaveDialog1->Execute()) {
		AnsiString target_file_name = SaveDialog1->FileName.c_str();
		ff = fopen(target_file_name.c_str(), "wt");
		if (ff) {
			AnsiString widget_text_value;
			widget_text_value = ComboBox1->Text.c_str();

			int kappa = atoi(widget_text_value.c_str());
			if (kappa >= 0) { // pt erou;
				widget_text_value = EditNumeErou->Text.c_str();

				std::string hero_name = widget_text_value.c_str();
				// Replace space with underscore
				std::replace(hero_name.begin(), hero_name.end(), ' ', '_');

				fprintf(ff, "^%s", hero_name.c_str());
				fprintf(ff, "\n^%d", kappa);
				char cifra = FelErou->Text.c_str()[1];
				widget_text_value = EditHeroLevel->Text.c_str();
				int lvl = atoi(widget_text_value.c_str());
				ScrieUnitate(ff, cifra - 48, lvl, 0);
				if (kappa >= 1) {
					char cifra = Merc1->Text.c_str()[1];
					widget_text_value = Edit1->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 1);
				}
				if (kappa >= 2) {
					char cifra = Merc2->Text.c_str()[1];
					widget_text_value = Edit2->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 2);
				}
				if (kappa >= 3) {
					char cifra = Merc3->Text.c_str()[1];
					widget_text_value = Edit3->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 3);
				}
				if (kappa >= 4) {
					char cifra = Merc4->Text.c_str()[1];
					widget_text_value = Edit4->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 4);
				}
				if (kappa >= 5) {
					char cifra = Merc5->Text.c_str()[1];
					widget_text_value = Edit5->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 5);
				}
				if (kappa >= 6) {
					char cifra = Merc6->Text.c_str()[1];
					widget_text_value = Edit6->Text.c_str();
					int lvl = atoi(widget_text_value.c_str());
					ScrieUnitate(ff, cifra - 48, lvl, 6);
				}
			}
			else
				MessageDlg("Datele nu au fost introduse corect", mtInformation,
...
void __fastcall TEditForm::OpenExistingHero1Click(TObject *Sender) {
	...
	if (OpenDialog1->Execute()) {
		AnsiString target_file_name = OpenDialog1->FileName.c_str();
		FILE *f = fopen(target_file_name.c_str(), "r");
		__citeste(f);
		char nume[25];
		fscanf(f, "%s", &nume);
		...
		for (int i = 0; i <= angajati; i++) {
			...
			switch (ImagineAsoc) {
			case 0:
				strcpy(buf, "(0)Peasant");
				break;
			case 1:
				strcpy(buf, "(1)Peasant Archer");
				break;
			case 2:
				strcpy(buf, "(2)Pikeman");
				break;
			case 3:
				strcpy(buf, "(3)Lizardman");
				break;
			case 4:
				strcpy(buf, "(4)Crossbowman");
				break;
			case 5:
				strcpy(buf, "(5)Dark Halberdier");
				break;
			case 6:
				strcpy(buf, "(6)Paladin");
				break;
			case 7:
				strcpy(buf, "(7)Longbowman");
				break;
			case 8:
				strcpy(buf, "(8)Magic Sprite");
				break;
			case 9:
				strcpy(buf, "(9)Magic Elemental");
				break;
			}
			switch (i) {
			case 0:
				FelErou->Text = buf;
				UpDownHero->Position = (short)Level;
				break;
			case 1:
				Merc1->Text = buf;
				UpDown1->Position = (short)Level;
